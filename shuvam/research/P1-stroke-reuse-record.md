...
**Motivation**: a literature review (kernel concentration theory, arXiv 2208.11060/2503.05602;
tabular-biomedical QSVM benchmarking, arXiv 2604.18837) surfaced two levers not yet tried on this
platform: (a) the angle-encoding bandwidth (the MinMax range feeding the ZZFeatureMap, previously
hardcoded to `±π/2`, now exposed as `experiment.run_experiment(..., angle_scale=...)`, a multiplier
on that half-width), motivated by exponential-concentration theory predicting an informative/
concentrated trade-off; and (b) using the quantum kernel's similarity-to-class-prototype as an
**engineered feature fed into classical `HistGradientBoostingClassifier`**, rather than running QSVC
standalone — motivated by a cardiovascular-risk paper reporting a real (if modest) ensemble gain from
this pattern. Both were tested here specifically because P1 found nothing from the second-pass
hyperparameter search (§9).

**(a) Bandwidth (`angle_scale`) sweep**, best known QSVC config (`C=0.1, reps=1, ent=circular`),
single-split screening (`max_train=300, max_test=150, seed=7`) across `angle_scale ∈ {0.15, 0.25,
0.4, 0.6, 0.8, 1.0, 1.5, 2.0, 3.0}`: noisy, non-monotonic (0.37–0.79 BA), with `angle_scale=1.0` (the
prior default) already the best point (0.787 — the same number §9 already found and already showed
fails production validation at 0.549). No off-default bandwidth improved on this. **No new candidate
to chase; not pursued further for P1.**

**(b) Quantum-kernel-prototype-similarity as a HistGB feature**, 10-seed repeated evaluation
(`max_train=300, max_test=150`, validation-carved `target_sensitivity=0.8` threshold policy applied
to *both* arms so the comparison is apples-to-apples and not an artifact of degenerate default
thresholding): classical-only (same ANOVA-reduced 4-feature set the platform's classical baseline
already uses) **0.6918 ± 0.1019** (n=10) vs. classical + 2 quantum-kernel-similarity features
**0.6615 ± 0.0743** (n=10). **The hybrid feature does not help — if anything, slightly worse, well
within noise.** A clean negative, not a broken/degenerate measurement (verified by first catching and
fixing a threshold-calibration bug in the prototype harness itself before trusting this number).

**Conclusion, updated**: P1 has now had three independent architectural levers tried in earnest
(hyperparameter/entanglement/ansatz sweep in §9; encoding bandwidth; quantum-kernel-as-feature
ensembling) and none moved it toward the baseline-viability gate. This is a well-searched, honest
negative result for this condition at this data scale (5110-row cross-sectional cohort, 4.87%
prevalence, low-confidence proxy labels per §3) — consistent with the broader literature finding
(arXiv 2604.18837) that classical kernels decisively dominate small tabular biomedical data. P1's QML
candidates remain `lifecycle: "experimental"`; no further architecture-search effort is planned for
this condition absent a new, qualitatively different lever or richer input data.

## 11. Fourth-pass — kernel alignment, VQC restarts, class weighting (2026-09-02)

Three more levers, surfaced by a second literature pass targeting P1's specific structure
(extreme 4.87% imbalance) and general quantum-kernel-concentration remediation:

**Quantum kernel alignment** (`qsvc_aligned`: `TrainableFidelityQuantumKernel` + `QuantumKernelTrainer`
with `svc_loss`, trains the feature-map rotation parameters via kernel-target alignment instead of
using the fixed ZZFeatureMap) — a real, previously-built engine capability
(`experiment._build_qsvc_aligned`), screened here for the first time against real data. Confirmed
**very expensive** (single-split screening at `max_train=100` took 386s — alignment training's cost
scales badly, consistent with the literature's "~2000x compute overhead" warning) and **not
promising**: single-split screening BA **0.5000** (degenerate). Not pursued further given the cost
and the lack of any positive screening signal to chase.

**VQC multi-restart** (`vqc_restarts=5`: fits 5 independently-initialized VQC candidates, keeps the
best training-accuracy one — mitigates COBYLA landing in a poor local basin) — single-split screening
BA **0.4583**, no improvement over the single-restart baseline. Not pursued further.

**Class-weighted (cost-sensitive) QSVC** (`class_weight="balanced"`, a new engine capability added
this session — `compute_sample_weight("balanced", y_train)` passed to `QSVC.fit(sample_weight=...)`,
following the well-established classical WSVM precedent for imbalanced classification) — motivated
specifically by P1's severe 4.87% prevalence. Single-split screening BA **0.5155**, well below the
unweighted baseline's 0.7867. Checked via 10-seed repeated evaluation anyway (screening alone can
mislead in the *optimistic* direction too, but a low screening number is much less likely to hide a
good repeated-eval result than the reverse) across C ∈ {0.1, 1.0, 5.0}: means **0.588±0.100**,
**0.522±0.111**, **0.532±0.104** — confirms no benefit, if anything worse than the unweighted
approach's already-failing 0.549.

**Conclusion, updated again**: P1 has now been tested against six independent architectural levers
across two research passes (hyperparameter/ansatz sweep, encoding bandwidth, quantum-kernel-as-
feature, kernel alignment, VQC restarts, class-weighting) with no improvement found from any of
them. This is a thoroughly, honestly searched negative result. P1's QML candidates remain
`lifecycle: "experimental"`.

## 12. Arm A finally built — real stroke MRI instead of risk factors (2026-09-03)

§10–11 concluded the limit was the *input*, not the circuit: 21 one-hot clinical risk factors
reduced to 4 features carry only so much signal, and no encoding trick creates more. This section
tests that directly by building the imaging arm this record deferred in its opening scope decision.

**Data**: **ISLES 2022** (Zenodo 7153326, **CC BY 4.0**, 1.69 GB, plain HTTP — no DUA, unlike the
CT/CTA/CTP sources checked in §4). 250 expert-annotated cases, BIDS layout, three sequences per
case: **DWI, ADC and FLAIR** — the diffusion imaging a stroke radiologist actually reads.

**Task framing.** Every ISLES case is a stroke patient, so the release has no negative class and a
"detect stroke" task cannot be built from it honestly. What it does support is the measurement that
actually drives treatment: **infarct core volume**. DAWN used core cutoffs of 21/31/51 mL and
DEFUSE-3 excluded cores ≥ 70 mL; automated core estimation from DWI is exactly what RAPID and
Viz.ai compute for thrombectomy triage. Labels are derived from the expert lesion mask's physical
volume (voxel count × header voxel size); **the model sees only DWI/ADC/FLAIR, never the mask.**

Measured core-volume distribution over all 250 cases (median 6.7 mL, mean 23.4 mL, max 482 mL) is
strongly right-skewed, so threshold choice matters and was made against the data rather than
assumed:

| cutoff | provenance | positives | share |
|---|---|---|---|
| 10 mL | not a trial cutoff | 109 | 43.6% |
| **21 mL** | **DAWN (age ≥ 80)** | **65** | **26.0%** |
| 31 mL | DAWN (< 80, NIHSS 10–19) | 48 | 19.2% |
| 51 mL | DAWN (< 80, NIHSS ≥ 20) | 30 | 12.0% |
| 70 mL | DEFUSE-3 exclusion | 22 | 8.8% |

DEFUSE-3's 70 mL leaves only 22 positives — too thin to evaluate honestly at this cohort size — so
**DAWN's 21 mL is the default**: an equally real published decision threshold with workable balance.

**Architecture** (`backend/src/qhealth_qml/imaging_hybrid.py`, `isles_stroke.py`,
`run_isles_hybrid.py`): volumes resampled to 64³ and percentile-normalised on ingest, then a 3-D CNN
(300k parameters) emits a 4-dimensional latent consumed by the *unchanged* classical and QSVC heads.
The compact 64³ cache (786 MB for all 250 cases × 3 channels) means the raw NIfTI tree can be
deleted after ingest — peak disk stays small regardless of collection size.

**Results** (leak-free: heads fitted on the encoder's train latents, thresholded on its validation
latents, scored only on its held-out test split; 3 independent seeds):

| Model | mean balanced accuracy | std | approx. 95% CI |
|---|---|---|---|
| `logistic_regression` | 0.8184 | 0.0542 | [0.6837, 0.9531] |
| raw 3-D CNN head | 0.8184 | 0.0542 | [0.6837, 0.9531] |
| `hist_gradient_boosting` | 0.8157 | 0.0502 | [0.6909, 0.9404] |
| `rbf_svc` | 0.8146 | 0.0489 | [0.6931, 0.9362] |
| **`qsvc` (decoupled)** | **0.8101** | **0.0427** | **[0.7041, 0.9162]** |

**Baseline-viability gate: CLEARED** by the QSVC (CI lower bound 0.704, far above 0.5) — the same
quantum configuration that fails on this condition's tabular arm. Against the tabular arm's
0.549 CI [0.439, 0.655], this is the second condition (with P6) where changing the *modality*
rescued a QSVC that six architecture-search levers could not.

**Honest caveats**: (a) this is **not the same question** as the tabular arm — that predicted
recorded stroke history from risk factors, this predicts large infarct core from diffusion imaging;
both are "P1 stroke" and the new one is far closer to the spec's intended arm A, but it is not a
like-for-like swap and the two numbers should not be quoted as a single before/after; (b) only 3
seeds, hence wide CIs — all five heads overlap heavily and **no quantum advantage is claimed**, only
gate clearance; (c) labels are derived from the same expert masks that define the ground truth, so
this measures how well core volume is *estimable from the images*, which is the real deployed task
but is not an independent clinical outcome; (d) the registry has not been updated — this is a new
dataset and modality and warrants its own `ModelDefinition` rather than an edit to the tabular entry.
