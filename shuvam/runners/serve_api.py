...
Endpoints:
  ...
  GET  /models                 catalogue: id, condition, temporal framing, expected inputs, limitations
...
* Every response states `temporal_framing` and `research_use_only`. A caller cannot render a
  characterisation score as an early-warning without ignoring an explicit field.
* Refusals are HTTP 200 with `status: "rejected"` and machine-readable `reasons`, not 500s —
  a rejected study is a normal, expected outcome that the caller must handle, not a server error.
* Model limitations travel in every prediction response, so they cannot be dropped by a UI that
  only reads the score.
...
class PredictRequest(BaseModel):
    ...
    Declared at module scope deliberately: FastAPI cannot resolve a BaseModel defined inside a
    closure, and silently demotes the body parameter to a required *query* parameter, which
    surfaces as a confusing HTTP 422 rather than an import error.
    """
...
def build_app(bundle_dir: Path):
    ...
    app = FastAPI(
        ...
        description=(
            ...
            "NOT a medical device. Every response carries its temporal framing and limitations."
        ),
        version="0.1.0",
    )
    ...
    def predict_paths(model_id: str, request: PredictRequest) -> dict[str, Any]:
        """Score a study whose files are already readable by the service."""
        ...
        ingested = ingest_study(MANIFESTS[model_id], dict(request.sources))
        if not ingested.ok:
            # a refusal is an expected outcome, not a server fault
            return {
                "model_id": model_id,
    ...
    async def predict_upload(model_id: str, files: list[UploadFile]) -> dict[str, Any]:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = predict(BUNDLES[model_id], ingested.volume, study_id="upload")
            result["ingestion"] = ingested.provenance
            return result
